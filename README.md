...
- **活动领取**：可领套餐一键领取（GUI 验证码）；设置里可开启**自动领取**（默认关）——轮询刷到可领套餐即自动完成验证并领取，成功 / 失败均有消息提示，验证失败后 30 分钟内不重复尝试
- **模型配置快照**：一键提取 / 置入账号的非内置模型 provider 配置（`zcode-models-v1` 快照），换号不丢自定义模型
- **切换保真**：切换前自动回同步漂移的登录态与模型配置、刷新套餐缓存；额度到期精确到分钟；OAuth 登录协议对齐官方 3.10.2；修复持锁切换死锁
