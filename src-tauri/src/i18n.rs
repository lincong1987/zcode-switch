...
const ZH: &[(&str, &str)] = &[
    ...
    ("err.models.config_object", "模型配置必须是 JSON 对象"),
    ("err.models.no_provider", "配置中没有 provider 对象"),
    ("err.models.invalid", "模型配置快照无效：{e}"),
    ("err.models.invalid_format", "不是有效的 zcode-models-v1 快照"),
    ("err.models.builtin", "模型快照不能包含内置 provider"),
    ("err.models.list", "读取模型配置目录失败：{e}"),
    ("err.models.mkdir", "无法创建模型配置目录：{e}"),
    ("err.models.no_active_account", "当前登录还没有保存为账号，无法提取或置入模型配置"),
    ("err.models.no_config", "当前没有 config.json"),
    ("err.models.no_custom_provider", "当前配置没有可提取的非内置 provider"),
    ("err.models.bad_file", "非法的模型配置文件名"),
    ("err.models.not_found", "模型配置文件不存在或已被删除"),
...
const EN: &[(&str, &str)] = &[
    ...
    ("err.models.config_object", "Model config must be a JSON object"),
    ("err.models.no_provider", "Config has no provider object"),
    ("err.models.invalid", "Invalid model config snapshot: {e}"),
    ("err.models.invalid_format", "Not a valid zcode-models-v1 snapshot"),
    ("err.models.builtin", "Model snapshots cannot contain builtin providers"),
    ("err.models.list", "Failed to read the model config directory: {e}"),
    ("err.models.mkdir", "Cannot create the model config directory: {e}"),
    ("err.models.no_active_account", "The current login is not saved as an account; model config cannot be extracted or injected"),
    ("err.models.no_config", "No config.json is present"),
    ("err.models.no_custom_provider", "The current config has no non-builtin provider to extract"),
    ("err.models.bad_file", "Invalid model config filename"),
    ("err.models.not_found", "The model config file does not exist or was deleted"),
...
mod tests {
    ...
    #[test]
    fn catalog_keys_parity() {
        let zh_keys: Vec<&str> = ZH.iter().map(|(k, _)| *k).collect();
        let en_keys: Vec<&str> = EN.iter().map(|(k, _)| *k).collect();
        assert_eq!(zh_keys.len(), en_keys.len(), "两表键数不一致");
        let mut zh_sorted = zh_keys.clone();
        let mut en_sorted = en_keys.clone();
        zh_sorted.sort_unstable();
        en_sorted.sort_unstable();
        assert_eq!(zh_sorted, en_sorted, "zh/en 键集存在差异");
        for table in [ZH, EN] {
            let mut keys: Vec<&str> = table.iter().map(|(k, _)| *k).collect();
            let before = keys.len();
            keys.sort_unstable();
            keys.dedup();
            assert_eq!(before, keys.len(), "该语言表内存在重复键");
        }
    }

    #[test]
    fn trf_substitutes_placeholders() {
        assert_eq!(tr_lang(Lang::Zh, "err.zcode.path_invalid"), "ZCode 路径无效：{p}");
        assert_eq!(
            trf_lang(Lang::En, "err.zcode.path_invalid", &[("p", "C:\\x")]),
            "Invalid ZCode path: C:\\x"
        );
    }

    fn trf_lang(lang: Lang, key: &str, args: &[(&str, &str)]) -> String {
        let mut s = tr_lang(lang, key);
        for (k, v) in args {
            s = s.replace(&format!("{{{k}}}"), v);
        }
        s
    ...
    #[test]
    fn coded_and_code_of_roundtrip() {
        let e = coded("wrong_password", "err.cipher.wrong_pw", &[]);
        assert_eq!(e, "wrong_password:密码错误或文件已损坏");
        assert_eq!(code_of(&e), Some("wrong_password"));
        assert_eq!(code_of("业务码 500: 没有资格"), None);
        assert_eq!(code_of("Quota API HTTP 401: x"), None);
        assert_eq!(code_of("无冒号错误"), None);
    }

    #[test]
    fn lang_parse_and_resolve() {
        ...
        assert_eq!(resolve(Some("fr")), resolve(None));
        match sys_locale::get_locale() {
            Some(os) => {
                let expect = if os.to_ascii_lowercase().starts_with("zh") { Lang::Zh } else { Lang::En };
                assert_eq!(resolve(None), expect, "os={os}");
            }
            None => assert_eq!(resolve(None), Lang::Zh),
        }
    }
    ...
    fn extract_literals(src: &str) -> Vec<String> {
        ...
        let b = src.as_bytes();
        let (mut st, mut i, mut cur) = (St::Code, 0, String::new());
        let mut out = Vec::new();
        while i < b.len() {
            match st {
                St::Code => match b[i] {
    ...
    #[test]
    fn used_i18n_literals_defined() {
        const PREFIXES: [&str; 6] = ["err.", "tray.", "title.", "dialog.", "cli.", "claim.fail."];
        let sources = [
            include_str!("lib.rs"),
            include_str!("store.rs"),
            include_str!("quota.rs"),
            include_str!("cipher.rs"),
        ...
        for src in sources {
            for lit in extract_literals(src) {
                if !PREFIXES.iter().any(|p| lit.starts_with(p)) {
                    continue;
                }
                if lookup(ZH, &lit).is_none() || lookup(EN, &lit).is_none() {
                    missing.push(lit);
                }
            }
        }
